...
#include <HV9808ESP32NixieDriver.h>
#include <SPI.h>
#ifdef ESP32
...
hw_timer_t *HV9808ESP32NixieDriver::timer = NULL;
#endif

HV9808ESP32NixieDriver *HV9808ESP32NixieDriver::_handler;

DRAM_CONST const uint32_t HV9808ESP32NixieDriver::nixieDigitMap[16] = {
	1,		// 0
	2,		// 1
	4,		// 2
	8,		// 3
 ...
 */
DRAM_CONST const uint32_t HV9808ESP32NixieDriver::colonMap[6] = {
	0,		// none
	0xf,	// all
	0x5,	// top
	0xa,	// bottom
...
uint32_t HV9808ESP32NixieDriver::currentColonMap[6] = {
	0,	// none
	0,	// none
	0,	// none
	0,	// none
...
uint32_t NIXIE_DRIVER_ISR_FLAG HV9808ESP32NixieDriver::getMultiplexPins() { return 0; }
uint32_t NIXIE_DRIVER_ISR_FLAG HV9808ESP32NixieDriver::getPins(byte mask) { return colonMap[mask]; }
uint32_t NIXIE_DRIVER_ISR_FLAG HV9808ESP32NixieDriver::getPin(uint32_t digit) { return transition == 1 ? 0 : nixieDigitMap[digit]; }
uint32_t NIXIE_DRIVER_ISR_FLAG HV9808ESP32NixieDriver::convertPolarity(uint32_t pins) { return pins ^ 0xffffffff; }
...
void HV9808ESP32NixieDriver::cacheColonMap() {
	if (indicator == 0) {
		for (int i=0; i<4; i++) {
			currentColonMap[i] = 0;
		}
...
void HV9808ESP32NixieDriver::setAnimation(Animation animation, int direction) {
	noInterrupts();
...
bool HV9808ESP32NixieDriver::animationDone() {
	noInterrupts();
...
bool NIXIE_DRIVER_ISR_FLAG HV9808ESP32NixieDriver::adjustAnimation(uint32_t nowMs) {
...
bool NIXIE_DRIVER_ISR_FLAG HV9808ESP32NixieDriver::esp32CalculateFade(uint32_t nowMs) {
...
void HV9808ESP32NixieDriver::init() {
	displayMode = NO_FADE;
	nextDigit = 0;
...
// in RAM.
void NIXIE_DRIVER_ISR_FLAG HV9808ESP32NixieDriver::isr() {
#ifdef ESP8266
    uint32_t ccount;
...
bool HV9808ESP32NixieDriver::calculateFade(uint32_t nowMs) {
	return true;
}

void HV9808ESP32NixieDriver::interruptHandler() {
}

//#define TEST_DRIVER
void NIXIE_DRIVER_ISR_FLAG HV9808ESP32NixieDriver::esp32InterruptHandler() {
#ifndef TEST_DRIVER
