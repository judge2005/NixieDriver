...
public:
	...
	inline void setNixieDigit(const byte digit) {
		nextDigit = digit;
	}
	inline void setNewNixieDigit(const byte digit) {
		startFade = millis();
		nextDigit = digit;
	}
...
protected:
	...
	bool ICACHE_RAM_ATTR calculateFade(unsigned long nowMs);

	volatile byte nextDigit = 0;
